...
export default async function ReviewSection(props:{productId:number, productRating:number,
	...
	const maxReview = Product.reviews?.reduce<MaxReview>(
  		(max, review, index) => {
  ...
  { value: -Infinity, index: -1, review: null }
);
	return (
		<div>
			<div className="w-full mb-5|| p-2">
				<div className="flex flex-col gap-2 mb-15">
					...
					productName={props.productName} productBrand={props.productBrand} Review={maxReview?.review}
					totalReview={Product.reviews?.length} backButton={false} />
