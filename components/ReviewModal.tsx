...
export default function ReviewModal({ isOpen, onClose, review }: ReviewModalProps) {
  ...
  const [isActive, setIsActive] = useState<boolean>(false);
    ...
    async function upvotes(reviewId: number, productId: number){
        setIsActive(true)
        const vote = await addUpvotes(reviewId, productId)
        if(vote){
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-0 md:p-4 md:pt-10 overflow-y-auto">
      {/* Container */}
      <div className="relative bg-[#F9F9F9] mt-30 md:bg-white w-full h-full md:h-auto md:max-w-[900px] md:rounded-[32px] shadow-2xl overflow-hidden transition-all duration-300 flex flex-col md:flex-row">
        ...
        <div className="  md:w-[55%] md:h-[50%]  p-6 md:p-15 flex items-center justify-center shrink-0">
        ...
        <div className="flex-1 px-4 pb-10 md:py-12 md:pr-12 md:pl-0 flex flex-col justify-center overflow-y-auto">
          ...
          <div className="bg-white md:bg-transparent p-6 md:p-0 rounded-[24px] md:rounded-none shadow-sm md:shadow-none">
            {/* User Info & Helpful */}
            <div className="flex justify-between items-start mb-2 md:mt-10">
              <div>
              ...
              <div className="flex items-center gap-1 text-[#2C2C2C] font-semibold">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={`w-5 h-5 ${isActive ? "animate-bounce" : ""}`}>
                ...
                <span className="text-[14px] cursor-pointer hover:opacity-60" onClick={() => upvotes(review.id, review.productId)}>Helpful ({review.numUpvotes})</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-[#505050] text-sm md:text-base leading-relaxed break-all overflow-scroll h-[20vh] md:h-40">
                {review.description}
              </p>
            </div>
          </div>
