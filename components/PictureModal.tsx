...
export default function PictureModal({ isOpen, onClose, review }: PictureModalProps) {
  ...
  return (
    <div 
      className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-0 md:p-4 overflow-y-auto"
      onClick={(e) => {
        e.stopPropagation();
        onClose();
      }}
    >
      {/* Container */}
      <div 
        className="relative bg-[#F9F9F9] md:bg-white w-[60%] md:h-auto md:max-w-[600px] md:rounded-[32px] shadow-2xl overflow-hidden transition-all duration-300 flex flex-col md:flex-row"
